...
namespace IIS.Product_19312
{
    ...
    public partial class UserE : BaseEditForm<Agent>
    {
        ...
        {
//            wolvUserRoles.SkipDataLoad = true;
        }
        ...
        protected override void Postload()
        {
            ...
            LoadingCustomizationStruct lcsOwnRoles = new LoadingCustomizationStruct(null);
            lcsOwnRoles.LoadingTypes = new[] { typeof(LinkRole) };
            lcsOwnRoles.LimitFunction = limitByAgent;
            View rolesView = Information.GetView("Sec_LinkRoleL", typeof(LinkRole));
            lcsOwnRoles.View = rolesView;

            List<LinkRole> userOwnRoles =
                BridgeToDS.GetDataService().LoadObjects(lcsOwnRoles).Cast<LinkRole>().ToList();
            ...
            if (IsPostBack)
            {
                ...
                foreach (LinkRole ownRole in userOwnRoles)
                {
                ...
                userOwnRoles = userOwnRoles.Except(deletedRoles).Union(linkRoles).ToList();
            ...
            List<LinkRole> parentRoles = new List<LinkRole>();
            GetParentRoles(userOwnRoles, rolesView, parentRoles);

            GenerateRolesTable(userOwnRoles, parentRoles);
        ...
        private void GenerateClassesTable(List<Permissions> permissions)
        {
            ...
            int i = 0;
            tblUserClasses.Rows.Add(tableHeaderRow);
            ...
            foreach (Permissions u2c in permissions)
            {
                ...
                tblUserClasses.Rows.Add(tableRow);
            }
        }

        private void GenerateRolesTable(List<LinkRole> userOwnRoles, List<LinkRole> parentRoles)
        {
            TableHeaderRow tableHeaderRow = new TableHeaderRow();
            tableHeaderRow.CssClass = "ObjectListViewCaptionCell";
            ...
            TableCell parentChecked = new TableCell();
            parentChecked.CssClass = "ObjectListViewCaptionCell";

            headName.Text = "Наименование";
            headChecked.Text = "Своё";
            parentChecked.Text = "Унаследовано";

            tableHeaderRow.Cells.Add(headName);
            tableHeaderRow.Cells.Add(headChecked);
            tableHeaderRow.Cells.Add(parentChecked);

            int i = 0;
            tblUserRoles.Rows.Add(tableHeaderRow);
            ...
            foreach (Agent role in _allRoles)
            {
                ...
                TableCell cellParent = new TableCell();
                cellParent.CssClass = "ObjectListViewCellValue";
                ...
                foreach (LinkRole ownRole in userOwnRoles)
                {
                    if (ownRole.Role.__PrimaryKey.ToString().Equals(role.__PrimaryKey.ToString()))
                    {
                        checkBox.Checked = true;
                    }
                }

                CheckBox parentBox = new CheckBox();
                ...
                foreach (LinkRole parentRole in parentRoles)
                {
                    if (parentRole.Role.__PrimaryKey.ToString().Equals(role.__PrimaryKey.ToString()))
                    {
                        ...
                        parentBox.Text += string.Format("{0}; ", parentRole.Agent.Name);
                    }
                }

                cellName.Text = role.Name;
                cellChecked.Controls.Add(checkBox);
                cellParent.Controls.Add(parentBox);

                tr.Cells.Add(cellName);
                tr.Cells.Add(cellChecked);
                tr.Cells.Add(cellParent);

                tblUserRoles.Rows.Add(tr);
            }
        }
        ...
        {
            DataObject.IsUser = true;

            if (!string.IsNullOrEmpty(ctrlPassword.Text))
            {
                DataObject.Pwd = new Checking().GetHashSHA1(ctrlPassword.Text);
            }
